...
if (!empty($data->Data_Iniziale) && !empty($data->Data_Finale) && !empty($data->Tipologia)) {
  ...
  $prestUnione->Tipologia = $data->Tipologia;

  // query products
  $stmt = $prestUnione->filter_date_type();
  $num = $stmt->rowCount();
  ...
  if ($num>0) {
    ...
    echo json_encode(array("message" => "Nessuna Prestazione Erogata trovata in Bonny della tipologia scelta nell'intervallo di date scelto."));
  }
} else {
  //400 bad request
